...
/**
 * Сформировать текстовую сводку данных для отправки в Claude API.
 */
...
/**
 * Отправить данные в Claude API для анализа.
 * @param {string} summaryText — текстовая сводка
 * @param {string} apiKey — ключ Anthropic API
 * @param {function} callback — function(error, analysisText)
 */
function requestAIAnalysis(summaryText, apiKey, callback) {
    var prompt = "Ты — аналитик по безопасности города Атырау, Казахстан. Работаешь в прокуратуре.\n\n" +
        "Проанализируй данные о преступлениях и дай подробный отчёт на русском языке:\n\n" +
        "1. ПРОБЛЕМНЫЕ ЗОНЫ — где концентрация преступлений максимальна и почему\n" +
        "2. ВРЕМЕННЫЕ ПАТТЕРНЫ — в какое время суток и дни недели чаще совершаются преступления\n" +
        "3. ТИПЫ ПРЕСТУПЛЕНИЙ — какие статьи лидируют и что это означает\n" +
        "4. ПРИЧИНЫ — проанализируй возможные причины криминогенной обстановки\n" +
        "5. РЕКОМЕНДАЦИИ — конкретные меры для снижения преступности:\n" +
        "   - Усиление патрулирования (где и когда)\n" +
        "   - Установка камер видеонаблюдения (конкретные локации)\n" +
        "   - Улучшение освещения\n" +
        "   - Социальные программы\n" +
        "   - Работа с населением\n" +
        "6. ТРЕНДЫ — растёт или снижается преступность, прогноз\n\n" +
        "Данные:\n" + summaryText;
    ...
    .then(function (data) {
        var text = data.content && data.content[0] ? data.content[0].text : "Нет ответа";
        callback(null, text);
    })
    .catch(function (err) {
        callback(err, null);
